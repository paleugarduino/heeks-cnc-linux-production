...
Soluzione: Cortocorticuitata la funzione `IsPasteReady()`.
...
il programma cercava gli ID grafici dei vecchi vettori ormai rimossi dalla memoria.
Un comando `assert()` rigido causava il crash immediato del software (`SIGABRT`).

Soluzione: Sostituto l'assert in `Index.h` con un controllo di sicurezza tollerante.
...
Per rigenerare i pacchetti puliti senza sporcare il sistema operativo ospite,
